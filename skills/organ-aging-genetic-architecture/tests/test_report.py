...
from presets import HEALTHY_N, LOCI, PAD_R_AFTER


def test_named_organs_keep_paper_order(tmp_path: Path):
    assert HEALTHY_N == 7217
    assert LOCI == 119
    assert PAD_R_AFTER == (-0.021, 0.025)
    text = run(tmp_path, "name,value\nkidney,named\nheart,named\nsmoking_initiation,yes\n")
    rows = listed(text)
    assert rows[0].startswith("1. 心脏")
    assert "心力衰竭" in rows[0]
    assert rows[1].startswith("2. 肾")
    assert "高血压" in rows[1]
    assert all(not row.startswith(tuple(f"{i}. 肺" for i in range(1, 5))) for row in rows)
    assert "吸烟起始" in text
    assert "没有算出器官生物年龄" in text
