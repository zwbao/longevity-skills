#!/usr/bin/env python3
"""Look up symbols in frozen APOE2 vs APOE3 pericyte significant proteome."""
...
def lookup(query, by_symbol, by_uniprot):
    ...
    return by_symbol.get(key) or by_uniprot.get(key)
...
def method_lines(queries):
    ...
    if not queries:
        ...
        return "请在 measurements 里交 symbol 或 uniprot。", lines
    hits = 0
    for q in queries:
        entry = lookup(q, by_symbol, by_uniprot)
        if entry is None:
            lines.append(f"- {q}：不在冻结的 {TABLE_SOURCE} 子集（{N_FROZEN} 个蛋白）里。")
            continue
        hits += 1
        lines.append(
            f"- {q} → {entry['gene']} ({entry['uniprot_id']})："
            f"AVG Log2 Ratio(APOE2/APOE3)={entry['log2_apoe2_over_apoe3']}；q={entry['qvalue']}。"
        )
    intro = (
        f"按 {TABLE_SOURCE} 查了 {len(queries)} 个标识符，命中 {hits} 个。"
        "这是 iPSC 周细胞蛋白质组标签，不是你的个人认知分数。"
    )
    return intro, lines
...
def cannot_lines():
    return [
        "## 这次算不了",
        "",
        "- 个人 Alzheimer 风险、周细胞功能分或脂滴定量。没有个人时钟系数。",
        "- File013 脂质显著表与重组 APOE2 剂量换算未冻进这次查表。",
        "- GNPC 血蛋白 APOE β（见 proteomic-apoe-alzheimers-signatures）不在本表。",
    ]
