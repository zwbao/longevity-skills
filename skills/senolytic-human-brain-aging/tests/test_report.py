...
from presets import BOUNDARY, GROUP_N, NAVITOCLAX_ORGANOID_UM
...
def test_public_counts():
    assert GROUP_N == 16
    assert NAVITOCLAX_ORGANOID_UM == 2.5

def test_rank_score():
    assert personal_report.rank_score(0.2, 2) == -0.2
    assert personal_report.rank_score(0.2, -3) == 0.2
    assert personal_report.rank_score(0.2, 0) == 0
...
def test_report_boundary_meds_and_labs(tmp_path: Path):
    ...
    assert "CDKN2A 的排序分是 -0.2" in text
    assert "不算转录组年龄" in text
    ...
    assert str(GROUP_N) not in text
