...
def test_ra_and_fatigue(tmp_path: Path):
    measures = tmp_path / "m.txt"
    measures.write_text("M10,3\nL5,1\nfatigue_effort,1\nfatigue_going,0\n", encoding="utf-8")
    text = personal_report.report(tmp_path / "o", 81, None, None, measures).read_text(encoding="utf-8")
    assert "相对振幅是 0.5000" in text
    assert "疲劳分是 1" in text
    assert "1022" not in text
    assert "衰弱表型：这次没有算" in text
