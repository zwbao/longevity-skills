...
ANSWERS = {key for key, _names, unit in MEASUREMENTS if unit == "score"}
...
def collect_inputs(measurements, age):
    ...
    duplicated, an answer other than yes or no, L5 above M10). A missing input
    is not a problem here; the report says what is missing, as before.
    """
    ...
    for row in rows:
        ...
        if hit and hit[0]["key"] in ANSWERS and token in YES | NO:
            row["value"] = "1" if token in YES else "0"
    ...
    for key in sorted(ANSWERS):
        answer = collected.values.get(key)
        if answer is not None and answer not in (0, 1):
            label = skillkit.spec_by_key(manifest, key)["label_zh"]
            problems.append(skillkit.Problem(key, label, "range", f"{label} 读成 {answer:g}。这一题只能答是（1）或否（0）。"))
    m10, l5 = collected.values.get("m10"), collected.values.get("l5")
    if m10 is not None and l5 is not None and l5 > m10:
        label = skillkit.spec_by_key(manifest, "l5")["label_zh"]
        problems.append(skillkit.Problem(
            "l5", label, "range",
            f"L5 是 {l5:g}，比 M10 的 {m10:g} 还大。最不活跃五小时的平均活动不会超过最活跃十小时，"
            "可能两项填反了，或者两项用了不同的单位。",
        ))
    values = {key: str(int(value)) if key in ANSWERS else repr(value) for key, value in collected.values.items()}
...
def report(out, age, medications, labs, measurements=None):
    ...
    if problems:
        ...
        skillkit.write_result(out, manifest, {"relative_amplitude": None, "fatigue_score": None})
        return path
    path = out / "report.md"
    path.write_text(render(age, medications, labs, values, bad), encoding="utf-8")
    skillkit.write_result(out, manifest, {
        "relative_amplitude": relative_amplitude(values),
        "fatigue_score": fatigue_score(values),
    })
    return path
