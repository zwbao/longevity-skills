...
def measurement_rows(path: Path | None) -> list[dict[str, str]]:
    ...
    name,value, now with an optional unit after the value. A path that does
    not exist counts as no measurements.
    """
...
def collect_inputs(measurements: Path | None, age: float | None) -> tuple[float | None, list]:
    """Read the age estimate and --age through skill.json: names, units, and ranges.

    Returns the age estimate in years and the problems that stop the
    subtraction: either number missing, a unit that cannot be converted, a
    value out of range, a value that is not a number, a duplicated row.
    """
    ...
    return collected.values.get("age_estimate"), problems
...
def write_report(out_dir: Path, measurements: Path | None, medications: Path | None, labs: Path | None, age: float | None) -> Path:
    ...
    if stale.exists():
        stale.unlink()
    manifest = skillkit.load_manifest(__file__)
    estimate, problems = collect_inputs(measurements, age)
    if problems:
        path = skillkit.write_problems(out_dir, problems, TITLE, BOUNDARY)
        text = path.read_text(encoding="utf-8").replace(
            "这次没有计算。原因如下：", "输入没有通过检查，所以年龄差不算。原因如下："
        )
        path.write_text(_with_paper_card(text), encoding="utf-8")
        skillkit.write_result(out_dir, manifest, {"age_estimate_error": None})
        return path
    text = render_report({"age_estimate": repr(estimate)}, load_lines(medications), load_labs(labs), age)
    path = out_dir / "report.md"
    path.write_text(_with_paper_card(text), encoding="utf-8")
    skillkit.write_result(out_dir, manifest, {"age_estimate_error": estimate - age})
    return path
