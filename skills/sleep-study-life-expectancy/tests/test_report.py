...
def test_boundary_medicine_labs_and_error(tmp_path: Path):
    ...
    text = personal_report.write_report(tmp_path / "full", measurements, meds, labs, 70).read_text(encoding="utf-8")
    assert text.splitlines()[-1] == f"边界: {BOUNDARY}"
    assert "不能据此停" in text
    assert "谷丙转氨酶 80 U/L" in text
    assert "年龄差 8 岁" in text
    assert "没有网络权重列" in text
    assert str(COHORT_N) not in text
    title = text.splitlines()[0]
    assert title.startswith("# ")
    assert not any(ch.isdigit() for ch in title)
    bare = personal_report.write_report(tmp_path / "bare", measurements, meds, None, 70)
