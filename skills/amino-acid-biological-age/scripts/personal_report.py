...
from presets import ALIASES, DISPLAY, EIGHT
...
def build(values, age):
    ...
    for key in EIGHT:
        ...
        for name, value in values.items():
            if ALIASES.get(name.strip()) == key or ALIASES.get(name.strip().lower()) == key:
                raw = value
                break
        if raw is None:
            missing.append(DISPLAY[key])
            continue
        chosen.append((key, float(raw)))
    if missing and not values:
        intro = ("# 氨基酸比例", "这次没有给出简化模型里的八个氨基酸浓度，所以不算比例，也没有算出 AmiAge。")
        return [], [], intro
    if missing:
        intro = (
            "# 氨基酸比例",
            "简化模型的比例是每个氨基酸除以这八个的浓度之和。这次没有给全，缺了"
            + "、".join(missing)
            + "，所以不算比例，也没有算出 AmiAge。",
        )
        return [], [], intro
    ...
    for key, value in chosen:
        ratio = value / total
        lines.append(f"{len(lines)+1}. {DISPLAY[key]}。浓度 {value:g}，占八个之和的 {ratio:.4f}。")
    intro = ("# 氨基酸比例", "这次把八个氨基酸各自的浓度除以这八个的和。没有算出 AmiAge。")
    return lines, [], intro
