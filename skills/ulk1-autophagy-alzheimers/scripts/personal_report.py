#!/usr/bin/env python3
"""ULK1 percent change. No model weights."""
...
def percent_change(baseline: float, follow: float) -> float:
    if baseline == 0:
        raise ZeroDivisionError("baseline is 0")
    return (follow - baseline) / baseline * 100.0


def _bag(rows: list[dict[str, str]]) -> dict[str, float]:
    found: dict[str, float] = {}
    for row in rows:
        keys = field_map(row)
        item = keys.get("item") or keys.get("项目") or keys.get("name")
        value = as_float(keys.get("value") or keys.get("结果"))
...
def method_lines(rows: list[dict[str, str]]) -> tuple[str, list[str], list[str]]:
    lines = [
        "## 方法算出的名单",
        "",
        "下面是论文细胞实验里的化合物，不是给你的剂量。",
    ]
    for name in COMPOUNDS:
        lines.append(f"- {name}。这是论文里的化合物，不是给你的剂量。")
    bag = _bag(rows)
    pairs = [
        ("血清", "serumbaseline", "serumfollow", SERUM_PERCENT_LOWER),
        ("脑脊液", "csfbaseline", "csffollow", CSF_PERCENT_LOWER),
    ]
    bits = []
    for label, base_key, follow_key, published in pairs:
        baseline = bag.get(base_key)
        follow = bag.get(follow_key)
        if baseline is None or follow is None or baseline == 0:
            continue
        change = percent_change(baseline, follow)
        bits.append(
            f"{label}从基线到随访的变化是 {change:.2f}%。"
            f"论文在认知未受损者的随访里写{label}低 {published}%。那是队列里的下降，不是给你的切点。"
        )
        lines.append(f"- {label} ULK1。变化 {change:.2f}%。")
    if not bits:
        intro = "这次没有同时给出基线和随访，所以没有算出 ULK1 的变化。"
    else:
        intro = "这次算出 ULK1 的变化。" + "".join(bits)
    return intro, lines, list(COMPOUNDS)
