...
from presets import BOUNDARY, DISCOVERY_WOMEN, PALB2_BRIDGES_YEARS
...
def run(tmp_path, measurements, labs=True):
    ...
    text = personal_report.report(tmp_path / "out", meas, meds, lab, 52).read_text(encoding="utf-8")
    ...
    assert DISCOVERY_WOMEN not in text
    assert PALB2_BRIDGES_YEARS not in text
    return text


def test_printed_effect_and_missing_beta(tmp_path: Path):
    text = run(tmp_path, "name,value\nZNF518A,HC-PTV\nPALB2,HC-PTV\nsex,female\n")
    title = text.splitlines()[0]
    assert title.startswith("# ")
    assert not any(ch.isdigit() for ch in title)
    rows = listed(text)
    assert "5.61" in rows[0]
    assert "早" in rows[0]
    assert "0.56" in rows[0]
    assert "1.38744" in rows[1]
    assert "标准误" in rows[1]
    assert "截距" in text
