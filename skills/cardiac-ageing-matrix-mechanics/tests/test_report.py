...
from presets import BOUNDARY, COHORT_UP_GENES
...
def test_report_boundary_meds_and_labs(tmp_path: Path):
    ...
    assert "不增删" in text
    assert personal_report.method_section(text) == personal_report.method_section(
        bare.read_text(encoding="utf-8")
    )
    assert "SoftY" in personal_report.method_section(text)
    assert "Acta2" in personal_report.method_section(text)
    assert str(COHORT_UP_GENES) not in text
