...
from presets import BOUNDARY, DEG_COUNT
...
def test_young_granulosa_and_foxp1(tmp_path: Path):
    text = run(tmp_path, "name,value\n颗粒细胞,named\nFOXP1,named\n", 22)
    assert text.splitlines()[0] == "# 人卵巢时空转录组"
    assert not any(ch.isdigit() for ch in text.splitlines()[0])
    body = "\n".join(listed(text))
    assert "年轻组" in body
    assert "GSTA1" in body
    assert "CDKN1A" in body
    assert "截距" in text
    outside = run(tmp_path, "name,value\n", 33)
    assert "不在正文的三个组" in "\n".join(listed(outside))
