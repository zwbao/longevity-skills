...
def test_report_boundary_meds_and_labs(tmp_path: Path):
    ...
    assert _opening(text) == "配套源码里没有拟合好的切片权重。这次也没有同时提供组织的预测年龄和实足年龄，所以没有算出年龄差。"


from presets import MAE_YEARS, R2, age_gap


def test_gap(tmp_path: Path):
    assert age_gap(70, 62) == 8
    assert MAE_YEARS == 4.88
    assert R2 == 0.69
    meas = tmp_path / "measurements.csv"
    meas.write_text("tissue,predicted,age,n_samples\nliver,70,62,50\n", encoding="utf-8")
    text = personal_report.report(tmp_path / "out", None, None, meas).read_text(encoding="utf-8")
    assert "肝：预测年龄减去实足年龄是 8.0 年" in text
    assert "功效不足" in text
    assert "4.88" in text
    assert "Zenodo" not in text
    assert "0.69" not in text
