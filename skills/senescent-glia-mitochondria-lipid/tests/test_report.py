...
from presets import BOUNDARY, LIPID_CELLS
...
def test_marker_and_climbing(tmp_path: Path):
    assert LIPID_CELLS == 100000
    ...
    blob = "\n".join(listed(text))
    assert "lamin B（Lam）" in blob
    assert "百分之 50" in blob
...
def test_blank_run(tmp_path: Path):
    ...
    assert "Supplementary Data 1" in text
