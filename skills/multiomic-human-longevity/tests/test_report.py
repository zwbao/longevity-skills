...
def test_named_genes_keep_paper_order(tmp_path: Path):
    ...
    rows = listed(text)
    assert rows[0].startswith("ZNF37A。")
    assert "TWAS Z 为正" in rows[0]
    assert rows[1].startswith("FLOT1。")
    assert "新的高置信基因" in rows[1]
    assert "NOTAGENE" not in "\n".join(rows)
    assert "NOTAGENE" in text
    assert "不能计算你的表观遗传年龄" in text
