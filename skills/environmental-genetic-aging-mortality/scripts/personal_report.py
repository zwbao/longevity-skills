...
from presets import (
    ENGLAND_N,
    EXPOSOME_EXTRA_MORTALITY_POINTS,
    EXPOSURES,
    EXPOSURES_REPLICATED,
    EXPOSURES_TESTED,
    PRS_EXTRA_MORTALITY_POINTS,
    XWAS_N,
)
...
def read_exposures(path):
    ...
    for line in Path(path).read_text(encoding="utf-8").splitlines():
        text = line.strip()
        if not text or text.startswith("#") or text.startswith("exposure"):
            continue
...
def report(args):
    answers = read_exposures(args.exposures)
    items = []
    for key, title, detail in EXPOSURES:
        mark = answers.get(key)
        if mark == "yes":
            items.append((title, f"你标成有。{detail}"))
        elif mark == "no":
            items.append((title, f"你标成没有。{detail}"))
        elif mark is not None:
            raise ValueError("present must be yes or no")
    if items:
        intro = "这次把你填写过的暴露对上最终簇模型里写明界限的项目。"
    else:
        intro = "没有填写暴露，这次没有对照。"
    meds = load_medications(args.medications)
    labs = parse_labs(args.labs) if args.labs else []
    text = render("暴露对照", intro, items, meds, labs, [])
...
def add_args(parser):
    parser.add_argument("--exposures", type=Path, default=None)
