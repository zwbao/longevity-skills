...
def build(values, age):
    del age
    geno = values.get("genotype", "").strip().lower()
    if geno in {"e2", "apoe2", "2"}:
        table = SIG["e2"]
        label = "APOE2 相对 ε3/ε3"
    elif geno in {"e4", "apoe4", "4"}:
        table = SIG["e4"]
        label = "APOE4 相对 ε3/ε3"
    else:
        intro = ("# 载脂蛋白蛋白名单", "这次没有给出 e2 或 e4，所以不按系数排序。")
        return [], [], intro
    hits = []
    for name, value in values.items():
        symbol = name.strip().upper()
        for row_symbol, protein_id, beta in table:
            if row_symbol == symbol:
                hits.append((abs(beta), row_symbol, protein_id, beta, value))
    hits.sort(key=lambda item: (-item[0], item[1], item[2]))
    lines = []
    for _, symbol, protein_id, beta, value in hits:
        lines.append(
            f"{len(lines)+1}. {symbol}（{protein_id}）。GNPC Model type 1，{label}的标准化系数是 {beta:.4f}。提供的数值是 {value}。系数不是这个人的效应。"
        )
    intro = (
        "# 载脂蛋白蛋白名单",
        "这次按论文标准化系数的绝对值，把你点到的蛋白从大到小排列。系数不是你的效应。",
    )
    return lines, [], intro
