...
from presets import BOUNDARY, COHORT_MARKER, ITEMS
...
def test_named_variants_keep_paper_order(tmp_path: Path):
    assert sum(1 for item in ITEMS if item["id"].startswith("rs")) == 49
    text = run(tmp_path, "name,value\nOSER1,named\nrs1474753,1\nrs000,0\n")
    rows = listed(text)
    assert rows[0].startswith("rs1474753。")
    assert "长寿组同向" in rows[0]
    assert rows[1].startswith("OSER1。")
    assert "rs000" not in "\n".join(rows)
    assert "比值比" in text
    assert "1.456" not in text
