...
def test_boundary_medicine_labs_and_no_probability(tmp_path: Path):
    ...
    assert "年龄系数" in text
    assert "一年死亡概率" in text
    assert str(COHORT_N) not in text
    title = text.splitlines()[0]
    assert title.startswith("# ")
    assert not any(ch.isdigit() for ch in title)
    bare = personal_report.write_report(tmp_path / "bare", measurements, meds, None, 60)
