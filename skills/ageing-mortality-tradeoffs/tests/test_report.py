...
def test_boundary_medicine_labs_and_locus(tmp_path: Path):
    ...
    text = personal_report.write_report(tmp_path / "full", measurements, meds, labs, 400).read_text(encoding="utf-8")
    assert text.splitlines()[-1] == f"边界: {BOUNDARY}"
    assert "不能据此停" in text
    assert "谷丙转氨酶 80 U/L" in text
    assert "Vita1a" in text
    assert "持续" in text
    assert "截距列" in text
    ...
    bare = personal_report.write_report(tmp_path / "bare", measurements, meds, None, 400)
