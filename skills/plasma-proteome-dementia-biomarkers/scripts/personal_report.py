...
from presets import ALIASES, MARKERS
...
def build(values, age):
    ...
    for name, value in values.items():
        ...
        raws[symbol] = value
    lines = []
    for symbol, direction in MARKERS:
        if symbol in supplied:
            lines.append(f"{len(lines)+1}. {symbol}。{direction}。提供的数值是 {raws[symbol]}。")
    if lines:
        intro_text = "这次只留下你测到、且正文写了方向的蛋白。不把蛋白加成一个分数。"
    else:
        intro_text = "这次没有对上正文写了方向的蛋白，所以不把蛋白加成一个分数。"
    intro = ("# 痴呆血浆蛋白", intro_text)
    return lines, [], intro
