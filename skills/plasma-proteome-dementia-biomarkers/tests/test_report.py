...
from presets import AD_N, AUC_FTD_AD, DISCOVERY_N


def test_marker_direction_not_a_score(tmp_path: Path):
    assert DISCOVERY_N == 374 + 532 + 196 + 216
    assert AD_N == 532
    assert AUC_FTD_AD == 0.83
    text = run(tmp_path, "name,value\nITGAV,2.1\nGFAP,8.4\nALB,40\n")
    rows = listed(text)
    assert rows[0].startswith("1. GFAP")
    assert "升高" in rows[0]
    assert rows[1].startswith("2. ITGAV")
    assert "降低" in rows[1]
    assert "ALB" not in "\n".join(rows)
    assert "不把蛋白加成一个分数" in text
