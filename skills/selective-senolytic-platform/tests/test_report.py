...
from presets import BOUNDARY, MGL392_MG_PER_MOUSE, NOTE3_VIABILITY
...
def test_public_counts():
    assert MGL392_MG_PER_MOUSE == 0.015
    assert NOTE3_VIABILITY == "130.8657"

def test_selectivity_index():
    assert personal_report.selectivity_index(8, 2) == 4
...
def test_report_boundary_meds_and_labs(tmp_path: Path):
    ...
    assert "选择性指数是 4" in text
    assert "没有拟合后的 IC50 列" in text
    ...
    assert NOTE3_VIABILITY not in text
