...
from presets import AGE_MODEL_IDS, PROTEIN_ALIASES, PROTEINS


NAMED = {title: True for title, _detail in PROTEINS}
MODEL_IDS = set(AGE_MODEL_IDS)
...
def resolve_protein(raw):
    token = raw.strip().lower()
    if token in MODEL_IDS:
        return token
    folded = fold_name(raw)
    for alias, display in PROTEIN_ALIASES:
        if fold_name(alias) == folded:
            return display
    return None
...
def read_proteins(path):
    ...
    for line in Path(path).read_text(encoding="utf-8").splitlines():
        ...
        if len(parts) < 2 or parts[0] in {"名字", "name", "protein", "uniprot"}:
            continue
...
def report(args):
    items = []
    seen = set()
    supplied = read_proteins(args.proteins)
    for raw, value in supplied:
        display = resolve_protein(raw)
        if display is None or display in seen:
            continue
        seen.add(display)
        if display in NAMED:
            detail = f"这次的数值是 {value}。它在正文点名、且与至少两种衰老相关代谢病有关的蛋白里。没有把这个数值换成风险，也没有计算健康衰老分数。"
        else:
            detail = f"这次的数值是 {value}。这个标识在年龄模型的蛋白公式里。没有训练矩阵，不算健康衰老分数。"
        items.append((display, detail))
    if not supplied:
        intro = "没有提供蛋白质数值，这次没有算出健康衰老分数。"
    elif items:
        intro = "这次记下你提供、且方法点过名的蛋白，没有计算健康衰老分数。"
    else:
        intro = "提供的名字不在方法点过名的蛋白里，这次没有计算健康衰老分数。"
    meds = load_medications(args.medications)
    labs = parse_labs(args.labs) if args.labs else []
    text = render("血清蛋白记录", intro, items, meds, labs, [])
...
def add_args(parser):
    parser.add_argument("--proteins", type=Path, default=None)
