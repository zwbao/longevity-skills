...
from presets import ALIASES, AOU_INPUTS, RETAINED


def token_of(name: str):
    text = name.strip()
    return ALIASES.get(text) or ALIASES.get(text.lower()) or text.lower().replace(" ", "_")
...
def build(values, age):
    ...
    lines = []
    seen = []
    for key, display in RETAINED:
        raw = None
        for name, value in values.items():
            if token_of(name) == key:
                raw = value
                break
        if raw is None or key in seen:
            continue
        seen.append(key)
        lines.append(f"{len(lines)+1}. {display}。提供的数值是 {raw}。")
    aou_hit = []
    for key in AOU_INPUTS:
        for name in values:
            if token_of(name) == key or name.strip().lower() == key:
                aou_hit.append(key)
                break
    if not values:
        intro_text = "这次没有对上论文保留下来的特征，也没有算出个人 EMRAge。"
    elif lines:
        intro_text = (
            f"这次对上了论文保留下来的 {len(lines)} 个特征。"
            f"All of Us 重建 EMRAge 时点名的输入对上了 {len(aou_hit)} 项。"
            "没有算出个人 EMRAge。"
        )
    else:
        intro_text = (
