#!/usr/bin/env python3
"""Record supplied dementia-linked proteins. The LightGBM risk score has no printed weights."""
...
def _values(rows: list[dict[str, str]]) -> dict[str, str]:
    ...
    for row in rows:
        ...
        value = keys.get("value") or keys.get("结果") or ""
        if item:
...
def method_lines(rows: list[dict[str, str]]) -> tuple[str, list[str], list[str]]:
    ...
    lines = ["## 方法算出的名单", ""]
    names = []
    for symbol, display in PROTEINS:
        value = None
        ...
        lines.append(f"- {symbol} {display}。你的数值是 {value}。这不是风险分。")
    if len(lines) == 2:
        lines.append("- 蛋白：这次没有对上。")
        intro = "风险分是梯度提升树，权重没有印出来，所以不算风险分。这次也没有对上论文点名的蛋白。"
    else:
        intro = "风险分是梯度提升树，权重没有印出来，所以不算风险分。对上的蛋白只记下数值。"
